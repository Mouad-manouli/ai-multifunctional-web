<x-master-users-component title="Setting">
    <form action="{{route('key')}}" method="POST">
        @csrf
        <input type="text" placeholder="Enter your secret key here..." class="form-control  input2  " name="key">
        @error('key')
            <div class="form-text text-danger ms-3">{{$message}}</div>
        @enderror
        <button class="btn btn-primary voice ms-1 mt-3"  type="submit">
